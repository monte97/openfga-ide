...
- [ ] Task 1: Create `AddTupleForm.vue` component (AC: #1, #2, #3)
  - [ ] Create `frontend/src/components/tuples/AddTupleForm.vue`
  - [ ] Three fields: User (`AppInput` monospace, placeholder `user:alice`), Relation (`AppSelect` populated from model store relations), Object (`AppInput` monospace, placeholder `document:roadmap`)
  - [ ] Submit button (`AppButton` primary, label "Add Tuple", loading state while submitting)
  - [ ] Zod schema for client-side validation: all three fields required, User and Object must match `type:id` pattern (non-empty strings containing `:`)
  - [ ] Validate on blur: show inline errors via `AppInput` error prop with `aria-describedby`
  - [ ] On successful submit: call `tupleStore.addTuple()`, show success toast ("Tuple added"), clear form fields
  - [ ] On submit error: keep form populated so user can retry (useApi already shows error toast)
  - [ ] Emit `added` event on successful creation so parent can refresh table

- [ ] Task 2: Extract model relations for Relation field (AC: #1)
  - [ ] In `AddTupleForm.vue` or a local computed: read `modelStore.json` and extract unique relation names from `type_definitions[].relations` keys
  - [ ] Map to `{ value: string, label: string }[]` format for `AppSelect`
  - [ ] If model is not loaded or has no relations, show empty AppSelect with placeholder "Load model first"
  - [ ] Ensure model store is imported and model is fetched (parent view handles fetch on mount)

- [ ] Task 3: Add delete button to each tuple row in TupleTable (AC: #4)
  - [ ] MODIFY `frontend/src/components/tuples/TupleTable.vue` (created by Story 3.2)
  - [ ] Add a "Delete" column as the last column in the TanStack Table definition
  - [ ] Each row renders a danger icon button (Trash2 from lucide-vue-next) in the delete column
  - [ ] On click: call `tupleStore.deleteTuple(tuple)`, which handles the DELETE call and toast
  - [ ] Button shows loading spinner while delete is in progress for that specific row

- [ ] Task 4: Add row selection checkboxes to TupleTable (AC: #5, #6)
  - [ ] MODIFY `frontend/src/components/tuples/TupleTable.vue`
  - [ ] Add a checkbox column as the first column (TanStack Table `enableRowSelection`)
  - [ ] Header checkbox for select-all / deselect-all
  - [ ] Track selected rows via TanStack Table's row selection state
  - [ ] Expose selected rows to parent via `defineExpose` or a `v-model:selectedRows` pattern

- [ ] Task 5: Add tuple CRUD actions to tuples Pinia store (AC: #2, #4, #6)
  - [ ] MODIFY `frontend/src/stores/tuples.ts` (created by Story 3.2)
  - [ ] Add `addTuple(storeId: string, tuple: { user: string, relation: string, object: string })` action: calls `api.post(\`stores/${storeId}/tuples\`, tuple)`, shows success toast ("Tuple added"), calls `fetchTuples()` to refresh
  - [ ] Add `deleteTuple(storeId: string, tuple: { user: string, relation: string, object: string })` action: calls `api.del(\`stores/${storeId}/tuples\`, body)` — NOTE: `useApi.del` currently sends no body; this needs a body-capable DELETE (see Dev Notes)
  - [ ] Add `deleteTuplesBatch(storeId: string, tuples: Array<{ user: string, relation: string, object: string }>)` action: calls `api.del(\`stores/${storeId}/tuples/batch\`, { deletes: tuples })`, shows success toast ("N tuples deleted"), calls `fetchTuples()` to refresh
  - [ ] All actions re-fetch tuples on success to ensure table shows fresh data

- [ ] Task 6: Extend `useApi.del` to support request body (AC: #4, #6)
  - [ ] MODIFY `frontend/src/composables/useApi.ts`
  - [ ] Change `del<T>(path: string)` signature to `del<T>(path: string, body?: unknown)`
  - [ ] When body is provided, add `headers: { 'Content-Type': 'application/json' }` and `body: JSON.stringify(body)` to the fetch options
  - [ ] Existing callers (StoreAdmin delete) pass no body, so backward compatible

- [ ] Task 7: Wire AddTupleForm and batch delete into TupleManager view (AC: #1, #2, #5, #6)
  - [ ] MODIFY `frontend/src/views/TupleManager.vue` (created by Story 3.2)
  - [ ] Add "Add Tuple" button in the header area that toggles visibility of the AddTupleForm
  - [ ] Render `AddTupleForm` above the table when visible; on `added` event, refresh table and optionally hide form
  - [ ] Add "Delete Selected" button (`AppButton` danger variant) that appears when rows are selected
  - [ ] On "Delete Selected" click: open ConfirmDialog with message "Delete N selected tuples?"
  - [ ] On ConfirmDialog confirm: call `tupleStore.deleteTuplesBatch()`, clear selection
  - [ ] Import and use `ConfirmDialog` component with variant="danger"
  - [ ] Ensure model store is fetched on mount (needed for AddTupleForm relation picker)

- [ ] Task 8: Tests (AC: #1-6)
  - [ ] Create `frontend/src/components/tuples/AddTupleForm.test.ts` — test form rendering, validation on blur, successful submit clears form, error keeps form populated
  - [ ] Modify `frontend/src/components/tuples/TupleTable.test.ts` — test delete button per row, checkbox selection, select-all
  - [ ] Modify `frontend/src/views/TupleManager.test.ts` — test Add Tuple toggle, Delete Selected with ConfirmDialog flow, batch delete
  - [ ] Modify `frontend/src/stores/tuples.test.ts` — test addTuple, deleteTuple, deleteTuplesBatch actions
